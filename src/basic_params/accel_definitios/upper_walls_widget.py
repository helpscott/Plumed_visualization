...
class WallsCVItem(QtWidgets.QWidget):
    ...
    def __init__(self, cv_outputs, parent=None):
        ...
        # 修改 placeholderText（中文说明）
        self.at_edit.setPlaceholderText("用于决定墙的位置 (默认示例 1.0)")
        # KAPPA
        self.kappa_edit = QtWidgets.QLineEdit()
        self.kappa_edit.setPlaceholderText("墙的力常数 (默认示例 150.0)")
        ...
        self.offset_edit.setPlaceholderText("墙的起点偏移量 (默认=0.0)")
        self.exp_edit = QtWidgets.QLineEdit()
        self.exp_edit.setPlaceholderText("幂次 (默认=2.0)")
        self.eps_edit = QtWidgets.QLineEdit()
        self.eps_edit.setPlaceholderText("缩放因子 (默认=1.0)")
...
class UpperWallsWidget(QtWidgets.QWidget):
    """
    界面逻辑：
    ...
    def __init__(self, cv_outputs, parent=None):
        ...
        self.cv_outputs = cv_outputs
        self.walls_items = []
        self.method_name = "upper_walls_1"

        main_layout = QtWidgets.QVBoxLayout(self)

        # 基础参数(此处仅展示 NUMERICAL_DERIVATIVES)
        ...
        walls_box = QtWidgets.QGroupBox("UPPER_WALLS - 选择多个ARG并设置参数")
    ...
    def get_definition_line(self):
        ...
        lines = [f"{self.method_name}: UPPER_WALLS ..."]
